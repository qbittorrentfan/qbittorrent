...
namespace
{
    ...
    void applyToTorrents(const QStringList &idList, const std::function<void (BitTorrent::Torrent *torrent)> &func)
    {
        ...
        else
        {
            ...
            {
                const auto hash = BitTorrent::InfoHash::fromString(idString);
...
void TorrentsController::infoAction()
{
    ...
    if (!hashes.isEmpty())
    {
        ...
        for (const QString &hash : hashes) {
            if (hash.length() > 40) {
                idSet->insert(BitTorrent::TorrentID::fromString(hash.left(40)));
        }  else {
...
void TorrentsController::propertiesAction()
{
    ...
    const auto hash = BitTorrent::InfoHash::fromString(params()[u"hash"_qs]);
    BitTorrent::Torrent *const torrent = BitTorrent::Session::instance()->findTorrent(hash);
    if (!torrent)
...
void TorrentsController::trackersAction()
{
    ...
    const auto infoHash = BitTorrent::InfoHash::fromString(params()[u"hash"_qs]);
...
void TorrentsController::uploadLimitAction()
{
    ...
    {
        int limit = -1;
        const BitTorrent::Torrent *const torrent = BitTorrent::Session::instance()->findTorrent(BitTorrent::InfoHash::fromString(id));
        if (torrent)
